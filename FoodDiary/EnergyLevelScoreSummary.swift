...
class EnergyLevelScoreSummary {
    var energyLevelScoreRange: String
    var mealCount: Int
    var entryCount: Int
    var energyLevelScoreRangeTotalCalories: Float
    ...
    init(entry: FoodDiaryEntry) {
        self.energyLevelScoreRange = entry.getEnergyLevelScoreRange()
        self.mealCount = 1
        self.entryCount = 1
        self.energyLevelScoreRangeTotalCalories = entry.calories
        ...
        if Session.sharedInstance.currentTotalCalories! == 0 {
            ...
            self.percentTotalCalories = self.energyLevelScoreRangeTotalCalories/Session.sharedInstance.currentTotalCalories!
        }
    }
    
    func updateEnergyLevelScoreRangeSummary(_ entry: FoodDiaryEntry) {
        ...
        self.energyLevelScoreRangeTotalCalories += entry.calories
        self.caloriesPerMeal = self.energyLevelScoreRangeTotalCalories/Float(self.mealCount)
        self.enjoymentScore += entry.enjoymentScore
        self.healthScore += entry.healthScore
        self.percentTotalCalories = self.energyLevelScoreRangeTotalCalories/Session.sharedInstance.currentTotalCalories!
        self.lastTimestamp = entry.timestamp as Date
